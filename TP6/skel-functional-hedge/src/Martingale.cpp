...
/**
 * @brief Apply a pre-treatment to lognormal data to obtain values in [-1,1]
 *
 * @param x input data (with a log normal distribution)
 * @param dim the index of the model
 * @param params An array of pairs (mean, var)
 * @return double
 */
static double normal_map(double x, int dim, void *params) {
    double* real_params = (double *) params;
    double mean = real_params[2 * dim];
    double var = real_params[2 * dim + 1];
    return -1 + 2 * pnl_cdfnor((std::log(x) - mean) / std::sqrt(var));
}

void ILocalStochasticIntegral::setBasisPreTreatment(const PnlMat * const *asset, int n_iterationsMC) {
    double *data_map = new double[2 * mod->m_modelSize];
    for (int dim = 0; dim < mod->m_modelSize; dim++) {
        double mean = 0, var = 0;
        for (int l = 0; l < n_iterationsMC; l++) {
            double log_x = std::log(MGET(asset[l], currentDate, dim));
            mean += log_x;
            var += log_x * log_x;
        }
        mean /= n_iterationsMC;
        var = var / n_iterationsMC - mean * mean;
        data_map[2 * dim] = mean;
        data_map[2 * dim + 1] = var;
    }

    pnl_basis_set_map(basis, normal_map, NULL, NULL, data_map, 2 * mod->m_modelSize * sizeof(double));
    delete [] data_map;
}
...
void ILocalStochasticIntegral::fit(const PnlMat * const*p_asset, const PnlVect *p_rhs) {
    ...
    setBasisPreTreatment(p_asset, n_iterationsMC);
    ...
    for (int l = 0; l < n_iterationsMC; l++) {
        ...
        // Use an inner conditioning to reduce variance
