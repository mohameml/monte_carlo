...
{
    if (!P.extract("model size", m_modelSize, true)) {
...
void Model::print() const
{
    ...
    cout << " Number of m_nTimeStepss : " << m_nTimeSteps << endl;
    cout << " Spot : ";
...
void Model::path(const PnlVect *drift)
{
    pnl_mat_clone(m_deltaB_drift, m_deltaB);
    for (int j = 0; j < m_brownianSize; j++) {
        const double drift_j = m_sqrt_dt * GET(drift, j);
        for (int i = 0; i < m_nTimeSteps; i++) {
            MLET(m_deltaB_drift, i, j) += drift_j;
        }
    }
...
void Model::path(PnlRng *rng, const PnlVect *drift)
{
    pnl_mat_rng_normal(m_deltaB, m_nTimeSteps, m_brownianSize, rng);
    path(drift);
}
...
void Model::path(const PnlMat *past, double t, const PnlVect *drift)
{
    int last_index = getLastTimeStep(t);
    pnl_mat_clone(m_deltaB_drift, m_deltaB);
    for (int j = 0; j < m_brownianSize; j++) {
        MLET(m_deltaB_drift, 0, j) += sqrt((last_index + 1) * m_dt - t) * GET(drift, j);
        const double drift_j = m_sqrt_dt * GET(drift, j);
        for (int i = 1; i < m_nTimeSteps - getLastTimeStep(t); i++) {
            MLET(m_deltaB_drift, i, j) += drift_j;
        }
    }
    path(past, t);
...
void Model::path(const PnlMat *past, double t, PnlRng *rng, const PnlVect *drift)
{
    pnl_mat_rng_normal(m_deltaB, m_nTimeSteps - getLastTimeStep(t), m_brownianSize, rng);
    path(past, t, drift);
}
...
void Model::shiftPath(int d, double fd_step, double t)
{
    int last_i = getNextTimeStep(t);
    for (int i = last_i; i <= m_nTimeSteps; i++) {
        MLET(m_pathMatrix, i, d) *= (1 + fd_step);
    }
}

void Model::unshiftPath(int d, double fd_step, double t)
{
    int last_i = getNextTimeStep(t);
    for (int i = last_i; i <= m_nTimeSteps; i++) {
        MLET(m_pathMatrix, i, d) /= (1 + fd_step);
