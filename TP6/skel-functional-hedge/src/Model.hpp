...
class Model
{
  public:
     ...
     *
     * @param rng PnlRng
     * @param ISDrift the importance sampling vector (same ISDrift for all time steps)
     */
    virtual void path(PnlRng *rng, const PnlVect *ISDrift);
    ...
    /**
     * Conditional sampling the model
     *
     * @param past a matrix containing the path up to time t (S_{t_0}, ..., S_{t_i}, S_t)
     * @param t current date
     * @param rng PnlRng
     * @param ISDrift the importance sampling vector (same ISDrift for all time steps)
     */
    virtual void path(const PnlMat *past, double t, PnlRng *rng, const PnlVect *ISDrift);
    ...
    /**
     * Set mod_path to a shifted path
     *
     * @param t current date
     * @param i index of the underlying to shift
     * @param fd_step magnitude of the shift x <- x / (1 + fd_step)
     */
    void shiftPath(int i, double fd_step, double t = 0);
    /**
     * Set mod_path to an unshifted path
     *
     * @param t current date
     * @param i index of the underlying to shift
     * @param fd_step magnitude of the shift x <- x / (1 + fd_step)
     */
    void unshiftPath(int i, double fd_step, double t = 0);
